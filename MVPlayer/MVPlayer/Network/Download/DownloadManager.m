...
/** 根文件夹 */
static NSString * const DownloadRootDir = @"com_520it_www_download";

/** 默认manager的标识 */
static NSString * const DowndloadManagerDefaultIdentifier = @"com.520it.www.downloadmanager";
...
{
    if (_file == nil) {
        _file = [[NSString stringWithFormat:@"%@/%@", DownloadRootDir, self.filename] prependCaches];
    }
...
- (DownloadState)state
{
    ...
    // 如果下载失败
    if (self.task.error) return DownloadStateNone;
...
- (void)notifyProgressChange
{
    ...
    [DownloadNoteCenter postNotificationName:DownloadProgressDidChangeNotification
                                        object:self
                                      userInfo:@{DownloadInfoKey : self}];
}
...
- (void)notifyStateChange
{
    ...
    [DownloadNoteCenter postNotificationName:DownloadStateDidChangeNotification
                                        object:self
                                      userInfo:@{DownloadInfoKey : self}];
}
...
- (void)cancel
{
    if (self.state == DownloadStateCompleted || self.state == DownloadStateNone) return;
...
- (void)willResume
{
    if (self.state == DownloadStateCompleted || self.state == DownloadStateWillResume) return;
...
- (void)suspend
{
    if (self.state == DownloadStateCompleted || self.state == DownloadStateSuspened) return;
...
+ (void)initialize
{
    _totalFileSizesFile = [[NSString stringWithFormat:@"%@/%@", DownloadRootDir, @"DownloadFileSizes.plist".MD5] prependCaches];
...
+ (instancetype)managerWithIdentifier:(NSString *)identifier
{
    ...
    DownloadManager *mgr = _managers[identifier];
    if (!mgr) {
        mgr = [self manager];
...
- (DownloadInfo *)download:(NSString *)url toDestinationPath:(NSString *)destinationPath progress:(DownloadProgressChangeBlock)progress state:(DownloadStateChangeBlock)state
{
    ...
    if (info.state == DownloadStateCompleted) {
        ...
        return info;
    } else if (info.state == DownloadStateResumed) {
        return info;
    }
...
- (void)resumeFirstWillResume
{
    ...
    DownloadInfo *willInfo = [self.downloadInfoArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"state==%d", DownloadStateWillResume]].firstObject;
    [self resume:willInfo.url];
}

- (void)cancelAll
{
    [self.downloadInfoArray enumerateObjectsUsingBlock:^(DownloadInfo *info, NSUInteger idx, BOOL *stop) {
...
- (void)resume:(NSString *)url
{
    ...
    // 正在下载的
    NSArray *downloadingDownloadInfoArray = [self.downloadInfoArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"state==%d", DownloadStateResumed]];
